...
- проверяем, что `lsof` показывает файл как удаленный:  
    ...
    ```  
- обнуляем дескриптор `10`, соответсвующий удаленному файлы. До и после удаления проверяем место на файловой системе `/`:  
    ```shell
    vagrant@test-netology:~/aaa
...
```  
Смысл использовать в bash `&&` при установленном режиме оболочки `set -e` есть, т.к. в случае использования `&&` для выполнения 
конвейера команд действие режима `set -e` не распространяется и выхода из оболочки не произойдет в случае 
возникновения ошибки в первой из двух команд в конвейере:  
```shell
vagrant@test-netology:~/aaa
> set +e
vagrant@test-netology:~/aaa
